...
Após a configuração, o Claude Code carrega automaticamente o `CLAUDE.md` em todas as conversas e disponibiliza os comandos através de `/ask-questions`, `/code-review` e `/commit-push`.

### Cursor

O [Cursor](https://cursor.com/) usa uma estrutura semelhante à do Claude Code:
...
2. **Para comandos de barra**: se o link simbólico ~/.cursor/commands tiver sido definido, os comandos de barra serão importados para o IDE Cursor e o cursor-agent.

```bash
# Comandos symlink para suporte a comandos de barra (/ask-questions, /code-review, /commit-push)
ln -s ~/src/ai-rules/commands ~/.cursor/commands
```

Após a configuração, invoque os comandos no chat do Cursor com `/ask-questions`, `/code-review` ou `/commit-push`.

### ChatGPT

O [ChatGPT](https://chat.openai.com/) não oferece suporte à configuração baseada em arquivo. Em vez disso, copie as regras para a configuração Instruções personalizadas:

1. Abra o ChatGPT e acesse `Configurações > Personalização > Personalizar ChatGPT`
2. Certifique-se de que a opção “Ativar personalização” esteja ativada
