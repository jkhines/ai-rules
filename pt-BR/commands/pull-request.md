...
7. **Crie o PR (corpo mínimo primeiro)**
   - Crie o PR imediatamente com um corpo mínimo para que o GitHub Actions comece a ser executado:
     ```
     gh pr create --base <base-branch> --title "<title>" --body "Descrição do PR pendente..."
     ```
   - Registre o número do PR na saída.

8. **Gerar descrição completa do PR**
   - Preencha o modelo de PR do repositório (em `.github/pull_request_template.md`) com o conteúdo da
     análise de diferenças na etapa 5. Preencha cada seção da seguinte maneira:
     - **Atividade do Jira** — Link para o ticket do Jira: `[<jira-id>](https://<jira-base-url>/browse/<jira-id>)`.
       Se não houver ticket Jira, escreva “Nenhum”.
...
- **Resultado esperado:** — Descreva o que o revisor deve observar após seguir as etapas de teste.
