...
alwaysApply: false
---
Analisa as alterações de código entre dois ramos. Por padrão, compara o ramo atual com o `main`. O usuário pode especificar explicitamente qualquer um dos ramos (por exemplo, `/code-review feature/auth dev`).

## Definições de gravidade

Classifique cada descoberta em exatamente um destes níveis:
...
| **Grave** | Causará comportamento incorreto, vulnerabilidade de segurança, degradação significativa do desempenho ou risco à integridade dos dados. Deve ser corrigido antes da fusão. | Condição de corrida em gravações simultâneas, falta de validação de entrada no limite da API externa, consulta N+1 em um endpoint de lista, erro ignorado silenciosamente, ocultando falhas dos operadores |
| **Menor** | Problema de qualidade de código, manutenção ou estilo que não afeta a correção de execução ou a segurança. A correção é desejável, mas não é bloqueante. | Nomenclatura inconsistente, falta de anotação de tipo em uma API pública, constante duplicada, nome de variável pouco claro |
| **Aprimoramento** | Sugestão de melhoria ou abordagem alternativa. Não é um defeito. | Sugestão de extrair um helper, proposta de um padrão mais idiomático, recomendação de adicionar uma métrica |

Em caso de dúvida entre dois níveis adjacentes, dê preferência à gravidade mais alta. Uma descoberta que se situe entre Crítico e Grave deve ser classificada como Crítica; uma que se situe entre Grave e Menor deve ser classificada como Grave.

## Processo

### 1. Resolver ramos e fazer fetch
Determine os ramos a serem comparados:
- **Ramo de recurso**: primeiro argumento posicional ou o ramo atual (`git branch --show-current`). Se o ramo atual for `main`, pergunte ao usuário qual ramo deve ser revisado.
- **Ramo base**: segundo argumento posicional ou `main`.

#### Resolução de referências
Para cada nome de ramificação, resolva-o em uma referência git concreta usando esta ordem de prioridade:
1. Se o HEAD estiver nessa ramificação e a referência local existir, use a referência **local** (para que commits não enviados sejam incluídos).
2. Caso contrário, se `origin/<name>` existir, use a referência **remota**.
3. Caso contrário, se a referência local existir, use-a.
4. Se nenhuma das duas existir, pare e relate o erro.

```bash
git fetch origin
# Resolver referências (exemplo: o branch de recurso é local, a base é remota)
git rev-parse --verify <feature>         # verificar local
git rev-parse --verify origin/<feature>  # verificar remota
git rev-parse --verify <base>
git rev-parse --verify origin/<base>
```

#### Calcular a diferença via merge-base
Use `git merge-base` para encontrar o ancestral comum e, em seguida, compare a partir desse ponto. Isso delimita corretamente a diferença apenas às alterações introduzidas no ramo de recurso, mesmo após rebases ou mesclagens na base.
...
Se `git merge-base` falhar, pare e relate o erro.

Para cada arquivo na saída do diff, verifique se as alterações reais existem com `git diff --quiet`. Ignore arquivos sem blocos de diff.
...
- Verifique a existência do arquivo: `git ls-tree -r --name-only <resolved-feature> -- <path>`
- Leia o conteúdo do arquivo: `git show <resolved-feature>:<path>`

Para cada arquivo alterado, leia o arquivo completo com `git show <resolved-feature>:<path>` para entender:
...
|----------|-------|
| Design | Adequa-se aos padrões arquitetônicos, evita acoplamento, separação clara de responsabilidades |
...
- **Impacto no produto**: O que isso oferece aos usuários/clientes
- **Abordagem de engenharia**: Padrões ou frameworks-chave utilizados
