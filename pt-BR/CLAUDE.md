...
- AWS: Use a CLI da AWS. Prefira os perfis nomeados em `~/.aws/config`: `sb` para sandbox, `dev` para desenvolvimento, `prod` para produção, e sempre passe `--profile <name>`. Use variáveis de ambiente (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`) apenas quando o perfil necessário não estiver disponível ou você for explicitamente orientado a usar credenciais de ambiente.
